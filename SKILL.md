...
`resolved` / `discarded` どちらも update sub-command が **close フロー** (時刻記録 + reason 正規化 + archive 移動 + 必要なら昇華先記録 + 未実装 DR の後続 issue 自動起票) を起動する。
...
update sub-command の close フローで `close_reason` を走査し、**`dr/*` 要素があり、かつ `implemented` が無い**場合、その DR 要素 1 つにつき 1 件、後続 issue を **同一リポに** write 経由で自動起票する (= 設計判断は記録したが実装が漏れる事故を防ぐ)。
...
- [DR-0001](docs/decisions/DR-0001-skill-over-hook-isolation.md) — issue 操作を sub-command に隔離する (commands 統一は DR-0003 で再判断)
