...
- ~~write skill は `${CLAUDE_SKILL_DIR}/templates/issue.md` を参照~~ → **DR-0003 で `${CLAUDE_PLUGIN_ROOT}/templates/issue.md` (plugin 直下) に移動**。template 変数は command 起動時に展開され fork 先 subagent prompt に渡るため fork 下でも Read できる
- fork 下での command 混在の共存、template の Read 可否、model 指定の効きは実機確認済 (v0.1.1 〜 v0.1.2 + cmux-msg 起票 dogfood で OK 確認、findings/2026-06-19-empty-command-body-issue.md 参照)
