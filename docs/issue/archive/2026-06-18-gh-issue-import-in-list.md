...
blocked_entered:
pending_entered:
discarded_entered: 2026-06-20T00:21:25+09:00
resolved_entered:
discard_reason: ["list の軽さ (haiku + fork で 1 秒応答) を優先", "gh→local 一方向同期でも認証管理・rate limit・network 障害対応のコストが大きい", "代替手段として /local-issue:write 経由でのクロスプロジェクト起票が write skill v0.1.1+ で実装済み = gh import を独自機能として実装する価値が低い", "双方向同期はさらに重く本 plugin の責務 (local docs/issue/ 管理) を超える"]
pending_reason:
close_reason:
blocked_by:
origin: 自リポ TODO
